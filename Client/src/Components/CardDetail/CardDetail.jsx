...
const CardDetail = () => {
  const dispatch = useDispatch()
  const [privatePass, setPrivatePass] = useState(null)
  const { showEvent, errorPass, eventDetail } = useSelector(state => state.eventDetail);

  console.log(eventDetail,'detalles de evento')
  ...
  useEffect(() => {
    if (privatePass) {
      dispatch(axiosGetEventPrivate({ id: eventDetail.id, privateEvent: privatePass }))
    }
  })
